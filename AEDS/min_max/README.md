...
<p align="center">Para a plotagem dos graficos que foram utilizados para melhor analissar os resultados deste estudo foi utilizada  a ferramenta <strong>GNUPLOT</strong> que eh  amplamente utilizado para representar funções matemáticas, dados experimentais e simulações numéricas. O Gnuplot é executado em uma variedade de plataformas, incluindo sistemas operacionais UNIX, Linux, Windows e macOS. Para a plotagem de graficos personalizados utilizando gnuplot uma das maneiras de realizar tal tarefa eh criando um <strong>script</strong> com a extensao <em>.gp</em> e fornecendo uma base de dados que sera utilizada para plotar os pontos e as linhas no grafico, atraves deste script o usuario da ferramenta consegue definir  diferentes caracteristicas  do  grafico e monta-lo da forma que melhor lhe seja conveniente. Logo apos, ainda no mesmo diretorio que esta localizado o  <strong>script</strong>  deve-se executar o  comando <min>gnuplot</min> seguido do nome do seu arquivo com a extensao <em>.gp</em>, tal comando ira gerar seu grafico da maneira com que voce definiu ao criar o <strong>script</strong>.   </p>
<h4 alig="center">Scripts utilizados</h4>
<p align="center">Neste projeto foram utilizados dois script`s diferentes para geracao dos graficos e que foram  alterados conforme as necessidades aparentes, ambos podem ser encotrados na pasta raiz deste repositorio nomeados como <em><a href="https://github.com/joaopaulocunhafaria/Faculdade/blob/main/AEDS/min_max/plot.gp">plot.gp</a></em> e <em><a href="https://github.com/joaopaulocunhafaria/Faculdade/blob/main/AEDS/min_max/plot2.gp">plot2.gp</a></em> . O primero script foi utilizado para gerar graficos com apenas uma linha enquanto o segundo foi utilizado para plotar graficos com mais de uma linha. </p>
**ATE AQUI
<hr>
